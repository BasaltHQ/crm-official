...
const UpdateTaskDialog = ({ users, boards, boardId, initialData, onDone }: Props) => {
  ...
  useEffect(() => {
    ...
    async function loadOpps() {
      if (!boardId) return;
      try {
        const res = await fetch(`/api/campaigns/${encodeURIComponent(boardId)}/opportunities`, { cache: "no-store" });
        if (res.ok) {
  ...
  const onSubmit = async (data: UpdatedTaskForm) => {
    ...
    try {
      ...
      await axios.put(`/api/campaigns/tasks/update-task/${initialData.id}`, data);
