...
function parseArgs() {
    ...
    const out = { region: process.env.AWS_REGION || 'us-west-2', wait: 10, max: 10, delete: false };
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === '--queue-url')
            ...
            out.wait = Number(args[++i]);
        else if (a === '--max')
            out.max = Number(args[++i]);
        else if (a === '--delete')
            out.delete = true;
    }
    if (!out.queueUrl)
...
async function main() {
    ...
    for (const m of msgs) {
        ...
        if (evt && evt['detail-type']) {
            ...
            if (detail) {
                ...
                const { action, voiceConnectorId, callId, streamArn, transactionId } = detail;
    ...
    if (doDelete) {
        ...
        const del = new client_sqs_1.DeleteMessageBatchCommand({ QueueUrl: queueUrl, Entries: entries });
